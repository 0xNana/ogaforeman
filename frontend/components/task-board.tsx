...
export function TaskBoard({ projectId, tasks, viewerId, onRefresh }: Readonly<{ projectId: string; tasks: Task[]; viewerId: string | null; onRefresh: () => Promise<void> }>) {
  ...
  return <div><PageHeader eyebrow="Work register" title="Tasks" description="Search the project work plan, inspect dependencies, and act on blocked work." actions={<><button className="btn btn-primary btn-small" type="button" onClick={() => setShowCreate(true)}><Plus size={15} aria-hidden="true" /> Add task</button>{isSetup && tasks.length ? <Link href={`/projects/${projectId}?setup=1`} className="btn btn-accent btn-small">Next step <ArrowRight size={15} aria-hidden="true" /></Link> : null}</>} />
    <div className="register-toolbar"><label className="register-search"><Search size={16} aria-hidden="true" /><span className="sr-only">Search tasks</span><input type="search" placeholder="Search tasks or assignees" value={query} onChange={(event) => { setQuery(event.target.value); setPage(1); }} /></label><div className="filter-tabs" aria-label="Task filters">{filters.map((item) => <button className={`filter-tab${filter === item.value ? ' active' : ''}`} type="button" aria-pressed={filter === item.value} onClick={() => changeFilter(item.value)} key={item.value}>{item.label}</button>)}</div><span className="register-count">{visible.length} {visible.length === 1 ? 'task' : 'tasks'}</span></div>
    {visible.length ? <><div className="data-table-wrapper"><table className="data-table register-table"><thead><tr><th>Task</th><th>Location</th><th>Trade</th><th>Assignee</th><th>Start</th><th>Due</th><th>Progress</th><th>Status</th></tr></thead><tbody>{rows.map((task) => <tr key={task.id}><th scope="row"><button className="register-row-link" type="button" onClick={() => setSelected(task)}>{task.title}</button></th><td>{task.location || 'Not recorded'}</td><td>{task.trade || 'Not recorded'}</td><td>{displayAssignee(task.assignee)}</td><td>{task.startLabel || 'Not set'}</td><td>{task.dueLabel}</td><td>{task.progress ?? 'Not reported'}{task.progress != null ? '%' : ''}</td><td><span className={`status-pill ${task.status.toLowerCase()}`}>{format(task.status)}</span></td></tr>)}</tbody></table></div><Pagination currentPage={page} totalItems={visible.length} pageSize={pageSize} onPageChange={setPage} /></> : <div className="empty-state"><span className="empty-state-icon"><ListTodo size={20} aria-hidden="true" /></span><h2>No matching tasks.</h2><p>Change the search or filter to see other project work.</p></div>}
    {selected ? <RecordDrawer eyebrow="Task details" title={selected.title} onClose={() => setSelected(null)}><span className={`status-pill ${selected.status.toLowerCase()}`}>{format(selected.status)}</span><RecordDetails items={[{ label: 'Location', value: selected.location || 'Not recorded' }, { label: 'Trade', value: selected.trade || 'Not recorded' }, { label: 'Assignee', value: displayAssignee(selected.assignee) }, { label: 'Start', value: selected.startLabel || 'Not set' }, { label: 'Due', value: selected.dueLabel }, { label: 'Progress', value: selected.progress == null ? 'Not reported' : `${selected.progress}%` }, { label: 'Dependencies', value: selected.dependencyIds?.length ? selected.dependencyIds.map((id) => taskNames.get(id)).filter(Boolean).join(', ') || 'Recorded task dependencies' : 'None recorded' }, { label: 'Blocker', value: selected.blocking || selected.note || 'None recorded' }, { label: 'Linked issue', value: selected.sourceRefs?.some((ref) => ref.startsWith('iss_')) ? 'Available in Issues' : 'None recorded' }, { label: 'Linked photos', value: 'Not available in this projection' }, { label: 'Source update', value: selected.sourceRefs?.some((ref) => ref.startsWith('sup_')) ? 'Available in Activity' : 'None recorded' }, { label: 'Activity', value: 'Open Activity for the project audit trail' }]} /></RecordDrawer> : null}
...
function matchesTask(task: Task, filter: TaskFilter, query: string, viewerId: string | null) {
  ...
  return !needle || [task.title, displayAssignee(task.assignee), task.location, task.trade].some((value) => value?.toLowerCase().includes(needle));
}

function format(value: string) { return value.toLowerCase().replaceAll('_', ' ').replace(/^./, (character) => character.toUpperCase()); }

function displayAssignee(value: string) {
  return /^usr_|^[A-Za-z0-9_-]{20,}$/.test(value) ? 'Project member' : value;
}
