...
export function IssueRegister({ issues, tasks }: Readonly<{ issues: Issue[]; tasks: Task[] }>) {
  ...
  const visible = useMemo(() => issues.filter((issue) => {
    ...
    return statusMatch && (!needle || [issue.description, issue.type, issue.owner, issue.location].some((value) => value?.toLowerCase().includes(needle)));
  }), [issues, query, status]);
  ...
  return <div><PageHeader eyebrow="Issue log" title="Issues" description="Track blockers, safety, quality, schedule risk, and site observations." />
    <div className="register-toolbar"><label className="register-search"><Search size={16} aria-hidden="true" /><span className="sr-only">Search issues</span><input type="search" placeholder="Search issues, owners, or locations" value={query} onChange={(event) => setQuery(event.target.value)} /></label><div className="filter-tabs" aria-label="Issue filters">{['OPEN', 'ACKNOWLEDGED', 'MITIGATED', 'RESOLVED', 'ALL'].map((item) => <button className={`filter-tab${status === item ? ' active' : ''}`} type="button" aria-pressed={status === item} onClick={() => setStatus(item)} key={item}>{format(item)}</button>)}</div><span className="register-count">{visible.length} {visible.length === 1 ? 'issue' : 'issues'}</span></div>
    {visible.length ? <div className="data-table-wrapper"><table className="data-table register-table"><thead><tr><th>Issue</th><th>Type</th><th>Location</th><th>Owner</th><th>Due</th><th>Status</th></tr></thead><tbody>{visible.map((issue) => <tr key={issue.id}><th scope="row"><button className="register-row-link" type="button" onClick={() => setSelected(issue)}>{issue.description}</button></th><td>{format(issue.type)}</td><td>{issue.location || 'Not recorded'}</td><td>{issue.owner}</td><td>{issue.dueLabel}</td><td><span className={`status-pill ${issue.status.toLowerCase()}`}>{format(issue.status)}</span></td></tr>)}</tbody></table></div> : <div className="empty-state"><span className="empty-state-icon"><MessageSquareWarning size={20} aria-hidden="true" /></span><h2>{issues.length ? 'No matching issues.' : 'Nothing blocking the site.'}</h2><p>{issues.length ? 'Change the search or filter. New issue records appear here when site evidence creates them.' : 'OG is watching for changes.'}</p></div>}
    {selected ? <RecordDrawer eyebrow="Issue details" title={selected.description} onClose={() => setSelected(null)}><span className={`status-pill ${selected.status.toLowerCase()}`}>{format(selected.status)}</span><RecordDetails items={[{ label: 'Description', value: selected.description }, { label: 'Status', value: format(selected.status) }, { label: 'Severity', value: format(selected.severity) }, { label: 'Type', value: format(selected.type) }, { label: 'Location', value: selected.location || 'Not recorded' }, { label: 'Responsible person', value: selected.owner }, { label: 'Due', value: selected.dueLabel }, { label: 'Linked task', value: selected.taskIds.length ? selected.taskIds.map((id) => taskNames.get(id)).filter(Boolean).join(', ') || 'Recorded project tasks' : 'None recorded' }, { label: 'Linked material', value: 'Not available in this projection' }, { label: 'Linked photos', value: 'Not available in this projection' }, { label: 'Source', value: selected.evidenceRefs.length ? 'Available in Activity' : 'None recorded' }, { label: 'Activity history', value: 'Open Activity for the project audit trail' }]} /></RecordDrawer> : null}
  </div>;
}
