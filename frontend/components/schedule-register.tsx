...
function matches(task: Task, filter: Filter, query: string) {
  ...
  return !needle || [task.title, task.trade, task.location, task.assignee].some((value) => value?.toLowerCase().includes(needle));
}
function linkedNames(ids: string[] | undefined, names: Map<string, string>) { return ids?.length ? ids.map((id) => names.get(id)).filter(Boolean).join(', ') || 'Recorded project activities' : 'None recorded'; }
