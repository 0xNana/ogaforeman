...
test('opens the deterministic product demo from the landing page', async ({ page }) => {
  ...
  await page.getByRole('link', { name: 'See OG in action' }).first().click();
  ...
  await expect(
    page.getByLabel('Interactive OG product demonstration'),
...
test('renders a keyboard-usable sign-up form', async ({ page }) => {
  ...
  await expect(page.getByRole('link', { name: /See OG in action without signing in/ })).toBeVisible();
  expect(browserErrors).toEqual([]);
});
