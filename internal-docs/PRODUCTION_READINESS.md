...
| PR-15 | Supplier simulation or an implicit logging notifier makes external coordination circular | Accept delivery delay only through authenticated operator intake. `NotificationService` may use logging only in local/test; staging and production explicitly require the sole real Google Chat provider, a typed payload, deployed secret configuration, persisted outbox claim, deterministic provider idempotency, bounded retry, terminal failure, and provider outcome. | Run provider contract/configuration, intake, outbox crash/retry, permanent-failure isolation, worker replay, and gated live Google Chat checks. Capture the real Chat message beside the correlated ADK/outbox telemetry. |
