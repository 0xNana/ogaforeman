...
| PR-09 | API workflow path bypasses the coordinator | Define one application entrypoint for event processing. API routes persist and publish events only; the worker invokes `OgaCoordinator`, which selects workflows. Direct workflow calls are test/demo adapters, not production paths. | API integration test proves a site update reaches the coordinator, creates one `AgentRun`, and follows the same path as Pub/Sub delivery. |
