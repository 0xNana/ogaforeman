...
DEPLOY_EVIDENCE_FILE="${DEPLOY_EVIDENCE_FILE:-artifacts/operations/${DEPLOY_ENVIRONMENT}-deployment-current.json}"
...
APP_VERSION="$(python3 -c 'import pathlib, tomllib; print(tomllib.loads(pathlib.Path("pyproject.toml").read_text(encoding="utf-8"))["project"]["version"])')"
GIT_STATUS="$(git status --porcelain)"
APP_SOURCE_TREE_DIRTY="false"
[[ -z "${GIT_STATUS}" ]] || APP_SOURCE_TREE_DIRTY="true"
IMAGE_TAG="${IMAGE_TAG:-${APP_GIT_SHA:0:12}}"
...
if [[ "${DEPLOY_DRY_RUN}" != "true" && "${ALLOW_DIRTY_DEPLOY}" != "true" ]] && \
  [[ "${APP_SOURCE_TREE_DIRTY}" == "true" ]]; then
...
for service_account in "${API_SERVICE_ACCOUNT_EMAIL}" "${WORKER_SERVICE_ACCOUNT_EMAIL}"; do
  run gcloud secrets add-iam-policy-binding "${CONVERSATION_PROPOSAL_SIGNING_SECRET}" \
    --project "${GOOGLE_CLOUD_PROJECT}" \
...
run gcloud builds submit \
  ...
  --substitutions "_IMAGE_URI=${IMAGE_URI},_APP_GIT_SHA=${APP_GIT_SHA},_APP_BUILD_TIME=${APP_BUILD_TIME},_APP_VERSION=${APP_VERSION}" \
  .
run gcloud builds submit \
  --project "${GOOGLE_CLOUD_PROJECT}" \
  --service-account "projects/${GOOGLE_CLOUD_PROJECT}/serviceAccounts/${BUILD_SERVICE_ACCOUNT_EMAIL}" \
  --config frontend/cloudbuild.yaml \
  --substitutions "_IMAGE_URI=${WEB_IMAGE_URI},_APP_GIT_SHA=${APP_GIT_SHA},_APP_BUILD_TIME=${APP_BUILD_TIME},_APP_VERSION=${APP_VERSION},_NEXT_PUBLIC_API_BASE_URL=${NEXT_PUBLIC_API_BASE_URL},_NEXT_PUBLIC_FIREBASE_API_KEY=${NEXT_PUBLIC_FIREBASE_API_KEY},_NEXT_PUBLIC_FIREBASE_AUTH_DOMAIN=${NEXT_PUBLIC_FIREBASE_AUTH_DOMAIN},_NEXT_PUBLIC_FIREBASE_PROJECT_ID=${NEXT_PUBLIC_FIREBASE_PROJECT_ID},_NEXT_PUBLIC_FIREBASE_STORAGE_BUCKET=${NEXT_PUBLIC_FIREBASE_STORAGE_BUCKET},_NEXT_PUBLIC_FIREBASE_MESSAGING_SENDER_ID=${NEXT_PUBLIC_FIREBASE_MESSAGING_SENDER_ID},_NEXT_PUBLIC_FIREBASE_APP_ID=${NEXT_PUBLIC_FIREBASE_APP_ID}" \
  .
...
ADK_AGENT_ENGINE_ID: '${ADK_AGENT_ENGINE_ID}'
...
APP_SOURCE_TREE_DIRTY: '${APP_SOURCE_TREE_DIRTY}'
EOF
...
run gcloud run deploy "${WEB_SERVICE}" \
  ...
  --set-env-vars "APP_GIT_SHA=${APP_GIT_SHA},APP_BUILD_TIME=${APP_BUILD_TIME},APP_VERSION=${APP_VERSION},APP_SOURCE_TREE_DIRTY=${APP_SOURCE_TREE_DIRTY}" \
  --ingress all \
  --allow-unauthenticated \
  --execution-environment gen2 \
  --cpu "${WEB_CPU}" \
